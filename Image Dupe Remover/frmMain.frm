...
Begin VB.Form frmMain 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.TextBox txtDirectory 
         ...
         Locked          =   -1  'True
         TabIndex        =   1
         Text            =   "Click on the start button on the right..."
         Top             =   360
         Width           =   5655
      End
   End
...
Sub process_file(fname As String)
    ...
    Else
        makedir txtDirectory & "\duplicate_images"
        If Option1.Value = True Then
            strDest = txtDirectory & "\duplicate_images\" & ftitle
            FileCopy fname, strDest
        End If
        Kill fname
        lstDupes.AddItem ftitle
    End If
    
    ' Count the number of files processed
    ' Doevents every 20 files
    iFCount = iFCount + 1
    If iFCount Mod 20 = 0 Then
        DoEvents
    End If
...
Private Function Compare(obj As String) As Boolean
    ...
    For i = 1 To lvMain.ListItems.Count
        If lvMain.ListItems(i).SubItems(1) = obj Then
            Compare = True
            lstDupes.AddItem lvMain.ListItems(i).Text & " matches"
            Exit Function
        End If
    Next i
End Function
...
Private Function FileList(ByVal Pathname As String, Optional DirCount As Long, Optional FileCount As Long) As String
    ...
    Do While first <> "" And EscapeKey = False
        process_file Pathname & first
        first = Dir
    Loop
End Function
